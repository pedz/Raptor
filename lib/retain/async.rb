...
module Retain
  ...
  # Combined models are created for each call and PMR and when fields
  # in those models are touched, more calls to Retain are fired.  This
  # can go on for some while.
  #
  ...
  #
  # The beanstalkd, async_observer, and beanstalk-client code (gems)
  # is used to provide the asynchronous ability.  The way that works
  # is AsyncObserver::Extensions will be mixed into this class.  The
  # async_send method is called on the object which serializes the
  # object along with the method to call (what is termed the
  # "selector"), sent to the "worker", which reconstructs the object
  # and executes the selector.
  class Async
  end
end
