class RetainServiceGivenCodesController < ApplicationController
  # GET /retain_service_given_codes
  # GET /retain_service_given_codes.xml
  def index
    @retain_service_given_codes = RetainServiceGivenCode.find(:all)
  ...
  def show
    @retain_service_given_code = RetainServiceGivenCode.find(params[:id])
  ...
  def new
    @retain_service_given_code = RetainServiceGivenCode.new
  ...
  def create
    @retain_service_given_code = RetainServiceGivenCode.new(params[:retain_service_given_code])

    respond_to do |format|
      if @retain_service_given_code.save
        flash[:notice] = 'RetainServiceGivenCode was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @retain_service_given_code.update_attributes(params[:retain_service_given_code])
        flash[:notice] = 'RetainServiceGivenCode was successfully updated.'
        format.html { redirect_to(@retain_service_given_code) }
        format.xml  { head :ok }
      else
